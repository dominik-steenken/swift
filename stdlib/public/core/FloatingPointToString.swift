...
// Stored as UInt16s for efficiency
#if _endian(big)
fileprivate let asciiDigitTable: _InlineArray<100, UInt16> = [
  ...
  0x3330, 0x3331, 0x3332, 0x3333, 0x3334,
  0x3335, 0x3336, 0x3337, 0x3338, 0x3339,
  0x3430, 0x3431, 0x3432, 0x3433, 0x3434,
  0x3435, 0x3436, 0x3437, 0x3438, 0x3439,
  0x3530, 0x3531, 0x3532, 0x3533, 0x3534,
  0x3535, 0x3536, 0x3537, 0x3538, 0x3539,
  0x3630, 0x3631, 0x3632, 0x3633, 0x3634,
  0x3635, 0x3636, 0x3637, 0x3638, 0x3639,
...
fileprivate func _infinity(
  ...
  if sign == .minus {
    var data: UInt32 = 0x666e692d // "-inf"
    #if _endian(big)
    ...
    var data: UInt32 = 0x00666e69 // "inf\0"
    #if _endian(big)
    data = data.byteSwapped
    #endif
    buffer.storeBytes(
...
fileprivate func _zero(
  ...
  if sign == .minus {
    var data: UInt32 = 0x302e302d // "-0.0"
    #if _endian(big)
    data = data.byteSwapped
    #endif
    buffer.storeBytes(
    ...
    var data: UInt32 = 0x00302e30 // "0.0\0"
    #if _endian(big)
    data = data.byteSwapped
    #endif
    buffer.storeBytes(
...
#if _endian(big)
  return (singles &+ 0x3030303030303030).byteSwapped
#else
